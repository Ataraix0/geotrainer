...
export default function EuropaMap() {
  ...
  // Build SVG once libs+countries ready
  ...
  // Recolor paths when state changes
  const colorMap = useCallback(() => {
    ...
    gRef.current.selectAll("path[data-name]").each(function() {
      ...
      if (mode === "explore") {
        ...
        else if (missed[name])         fill = "#5a2030";
      }
  ...
  const attachHandlers = useCallback(() => {
    ...
    gRef.current.selectAll("path[data-name]")
      ...
      .on("mouseenter", function(event, d) {
        const name = d?.properties?.name || this.getAttribute("data-name") || "";
        if (mode === "explore") {
          if (!seen[name]) d3.select(this).attr("fill", C.landHov);
          setHovered({ name: getES(name), cap: getCap(name), flag: getFlag(name) });
        } else {
          d3.select(this).attr("fill", C.landHov);
        }
      })
      .on("mouseleave", function(event, d) {
        const name = d?.properties?.name || this.getAttribute("data-name") || "";
        if (mode === "explore") {
          d3.select(this).attr("fill", seen[name] ? C.correct : C.land);
        } else {
          const fill = correct.includes(name) ? C.correct : missed[name] ? "#5a2030" : C.land;
  ...
  const accuracy    = (correct.length + totalErrors) > 0
    ? Math.round(correct.length / (correct.length + totalErrors) * 100) : 100;
      ...
      <div style={{ display:"flex", gap:6, padding:"10px 0 8px", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap" }}>
        <div style={{ display:"flex", gap:6 }}>
          {[["pin","📍 PIN"],["explore","🔍 Explorar"]].map(([m,l]) => (
            <button key={m} onClick={() => { setMode(m); setPhase("start"); clearInterval(timerRef.current); setSeen({}); setHovered(null); }}
              style={{ padding:"6px 16px", borderRadius:20, border: mode===m ? `1px solid ${C.gold}` : `1px solid ${C.border}`,
                background: mode===m ? "rgba(233,196,106,0.15)" : "transparent",
      ...
      <div style={{ border:`1px solid ${C.border}`, borderRadius:14, overflow:"hidden", background: C.ocean }}>
        ...
        {mode==="pin" && phase==="start" && (
          <div style={panelStyle}>
            <div>
              <div style={{ color: C.gold, fontWeight:600, fontSize:15, letterSpacing:"0.05em" }}>Europa · Países</div>
              <div style={{ color: C.muted, fontSize:12, marginTop:2 }}>{countries.filter(c=>COUNTRY_DATA[c.properties?.name]).length} países · haz clic en el país correcto</div>
            </div>
        ...
        {mode==="pin" && phase==="playing" && target && (
          <div style={panelStyle}>
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              ...
              <div>
                <div style={{ fontSize:11, color: C.muted, letterSpacing:"0.1em", textTransform:"uppercase" }}>Encuentra</div>
                <div style={{ fontSize:20, fontWeight:700, color:"#fff", letterSpacing:"0.02em" }}>{getES(target)}</div>
              </div>
            </div>
        ...
        {/* PIN — progress bar */}
        {mode==="pin" && phase==="playing" && (
          <div style={{ height:3, background:"rgba(255,255,255,0.06)" }}>
            <div style={{ height:"100%", width: queue.length ? (correct.length/queue.length*100)+"%" : "0%",
        ...
        {mode==="pin" && phase==="playing" && remaining.length > 0 && (
          <div style={{ background:"rgba(255,255,255,0.03)", padding:"8px 14px", borderTop:`1px solid ${C.border}` }}>
            <div style={{ fontSize:10, color:C.muted, letterSpacing:"0.1em", marginBottom:5 }}>PENDIENTES ({remaining.length})</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:"3px 10px" }}>
              {remaining.map(n => (
                <span key={n} style={{ fontSize:12, color:"rgba(232,224,208,0.55)" }}>{getES(n)}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {/* RESULT */}
        {phase==="result" && (
          <div style={{ padding:"24px 20px", textAlign:"center", background:"linear-gradient(135deg,#0f0c29,#16213e)", borderTop:`1px solid ${C.border}` }}>
            <div style={{ fontSize:36, marginBottom:10 }}>🌍</div>
            <div style={{ fontSize:20, fontWeight:600, color:"#fff", marginBottom:6 }}>¡Completado!</div>
            <div style={{ display:"flex", justifyContent:"center", gap:24, marginBottom:16 }}>
              {[["Tiempo", fmtTime(timer), C.gold],["Aciertos", correct.length, C.correct],["Errores", totalErrors, totalErrors?C.wrong:C.correct],["Precisión", accuracy+"%", accuracy>=90?C.correct:accuracy>=70?C.gold:C.wrong]].map(([l,v,c]) => (
                <div key={l}>
                  <div style={{ fontSize:10, color:C.muted, letterSpacing:"0.08em" }}>{l.toUpperCase()}</div>
                  <div style={{ fontSize:20, fontWeight:700, color:c }}>{v}</div>
                </div>
              ))}
            </div>
            {Object.keys(missed).length > 0 && (
              <div style={{ marginBottom:16, background:"rgba(230,57,70,0.1)", borderRadius:10, padding:"10px 14px", border:"1px solid rgba(230,57,70,0.2)", textAlign:"left" }}>
                <div style={{ fontSize:10, color:C.wrong, letterSpacing:"0.08em", marginBottom:6 }}>FALLASTE EN</div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:"4px 12px" }}>
                  {Object.keys(missed).map(n => (
                    <span key={n} style={{ fontSize:13, color:"rgba(232,224,208,0.8)" }}>{getFlag(n)} {getES(n)}</span>
                  ))}
                </div>
              </div>
            )}
